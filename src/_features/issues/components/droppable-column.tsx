...
export function DroppableColumn({ id, issues, projectId, color, limit, isMobile = false }: DroppableColumnProps) {
  ...
  return (
    ...
    >
            <div
              ref={setNodeRef}
              className={`flex flex-col gap-2 md:gap-3 h-full rounded-b-lg md:rounded p-3 md:p-3 overflow-y-auto transition-all duration-200 ${
                isLimitExceeded ? "opacity-60" : ""
              }`}
              style={{
                ...getColumnStyle(),
                touchAction: "pan-y", // 세로 스크롤만 허용, 드래그는 센서가 처리
              }}
            >
        {issues.length === 0 ? (
          <Card
          ...
          >
            <CardContent className="flex items-center justify-center py-12 text-sm text-[#6B778C] font-medium">
              {isMobile ? "이슈가 없습니다" : isOver ? "여기에 놓으세요" : "이슈를 드래그하세요"}
            </CardContent>
          </Card>
        ) : (
          <>
